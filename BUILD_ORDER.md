...
Did not: re-score the 151 notices already scored before this landed — their old `estimated_value_usd` was dropped (migration 012), not recomputed, because it was never read from the text correctly to begin with. Did not decide the export basis — `value_basis` defaults to `published`, both branches it can take are built and tested, and which one BD wants on the Zoho side is still open. Did not resolve BOAMP's 24 lot-only notices or Prozorro's VAT-inclusive/exclusive split; both are carried as published and read nowhere else.
