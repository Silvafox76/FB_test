...
| The register's class A wave | **Three more national sources live on 2026-09-19** (decision 72): Contracts Finder (GB below threshold), Bosnia and Herzegovina's EJN OpenAPI, Latvia's IUB daily files; Public Contracts Scotland built and held on a TLS trust-bundle decision. Fourteen onboarding runs from the 91-row Source Access and ToS Register; ten decision points recorded in the decisions artefact; the 40 class B rows carry one decision each in the register workbook (decision 73). BD's account list read against the register: two rows matter (UNGM's corporate mailbox, Georgia's login), the aggregators stay out, and the rule for reading through an existing corporate account is a sentence on rule 21 (decision 74). Georgia and UNGM assessed the same evening: Georgia's portal is robots disallow-all behind a login redirect, UNGM's alert service is under the same compilation ban as its free site; both wait on a letter, not a build. |
