...
26. **Region status is config, changed by commit.** A region's status (`inactive`, `onboarding`, `build`, `shadow`, `live`) lives in `config/regions.yaml` and changes only in a commit whose message cites the gate evidence. A global mode flag, a status set from the CLI or the database, or two regions in `build` or `shadow` at once is blocking.
...
21. Acquisition ethics: respect robots.txt, identified user agent (`FreeBalance-OpportunityMonitor/0.1 (+contact email)`), one polite pass per schedule, no CAPTCHA solving, no residential or rotating proxies, no third-party mirror of an official portal, no user agent that disguises the client. An existing FreeBalance account may be used to read a source only when it is a corporate identity, the portal's terms permit automated reading with it, the credential sits in the environment or Secrets Manager, and login is username and password with no CAPTCHA or second factor; the source YAML records the identity, never the secret (decision 74).
...
- **There is no automated duplicate check and there will not be one in this pilot.** The candidate page's decision panel carries a standing reminder to check the CRM by hand for an existing Opportunity on the buyer. This is proven necessary, not theoretical: the Ghana tender the Monitor would surface already has a live Opportunity from December 2025. Under D31 the reviewer is the only duplicate control, every duplicate that reaches an export batch is logged as a finding, and the count is a week 14 gate number. For the pilot the check is not asked for at approval (decision 69, 19 September 2026): the reviewer makes one of two decisions, approve or reject, an approval may carry the tag `monitor`, and the export page is where a person reconfirms each waiting record and includes or excludes it from the batch. The duplicate question returns with the CRM question after the gate.
